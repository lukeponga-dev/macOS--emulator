        ...
        {tabs.map((tab) => (
          <div
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs cursor-default max-w-[200px] ${
              activeTab === tab.id ? "bg-white shadow-sm" : "hover:bg-black/5"
            }`}
            onClick={() => {
              setActiveTab(tab.id)
              setUrl(tab.url)
        ...
        {currentTab?.url ? (
          <div className="h-full flex items-center justify-center bg-gradient-to-b from-gray-50 to-white">
            <div className="text-center">
              <div className="text-6xl mb-4">{currentTab.favicon}</div>
              <h2 className="text-xl font-semibold mb-2">{currentTab.title}</h2>
              <p className="text-sm text-gray-500">{currentTab.url}</p>
